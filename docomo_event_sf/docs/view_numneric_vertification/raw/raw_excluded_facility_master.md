# raw_excluded_facility_master 数値検証

## 1. メタ情報
- 実施日: 2026-07-04
- 実施者: numerical-verification skill
- 検証コード: `docomo_event_sf/tools/verify_code.py`（再現コマンド: `python tools/verify_code.py --table 除外施設マスタ`）

## 2. 対象
- 比較元（Snowflake）: `HARATO.RAW.RAW_EXCLUDED_FACILITY_MASTER`
- 比較先（BigQuery）: `digital-well-456700-i9.docomo_event_raw.raw_excluded_facility_master`
- スコープ: 全件
