# int_facility_event_decile_avg_actual 数値検証（デシル平均実績）
...
- 検証コード: `docomo_event_sf/tools/verify_code.py`（再現コマンド: `python tools/verify_code.py --table デシル平均実績`）

## 2. 対象
- 比較元（Snowflake）: `DOCOMO_DB.INT.INT_FACILITY_EVENT_DECILE_AVG_ACTUAL`（VIEW）
...
- BigQuery: `d3480196258f5eaa09e31df8096e7a8f`
- Snowflake: `37eab1b2cf486a47ccccf261d015ef1d`
- 一致: ✗（既知の許容差分による不一致）
...
## 8. 判定
- 判定: **OK**（既知の許容差分のみ）
- 補足: 差分9行はすべて `avg_actual` の丸め1単位ずれ（要因B）。キー列・件数・その他列は完全一致。
