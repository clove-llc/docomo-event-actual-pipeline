...
対象（既定）:
  人流デシル     : BQ raw_facility_foot_traffic_avg_and_decile_by_flag ↔ SF STG.STG_FACILITY_DECILE_BY_FLAG
  季節指数       : BQ raw_facility_daily_deviation_zscore             ↔ SF STG.STG_FACILITY_DAILY_DEVIATION_ZSCORE
  実績データ     : BQ raw_facility_actuals                            ↔ SF STG.STG_FACILITY_ACTUALS
  日付マスタ     : BQ raw_date_master                                 ↔ SF RAW.RAW_DATE_MASTER
  施設マスタ(5列): BQ raw_facility_master                             ↔ SF RAW.RAW_FACILITY_MASTER
  施設名マッピング: BQ raw_facility_name_mappings                      ↔ SF RAW.RAW_FACILITY_NAME_MAPPINGS
...
使い方:
  python tools/verify_code.py                 # 全テーブル突合
  python tools/verify_code.py --table 季節指数  # 1テーブルのみ
...
BQ_DATASET = os.environ.get("BQ_DATASET", "docomo_event_raw")
...
# (ラベル, BQテーブル, SF FQTN, 列型, 除外列{列: 理由})
TABLES = [
    ("人流デシル", "raw_facility_foot_traffic_avg_and_decile_by_flag", "HARATO.STG.STG_FACILITY_DECILE_BY_FLAG", T_DECILE, {}),
    ("季節指数", "raw_facility_daily_deviation_zscore", "HARATO.STG.STG_FACILITY_DAILY_DEVIATION_ZSCORE", T_ZSCORE, {}),
    ("実績データ", "raw_facility_actuals","HARATO.STG.STG_FACILITY_ACTUALS", T_ACTUALS, {"source_sheet_name": "SF=正規化yyyymm(202510) / BQ=生シート名(2025.1) の設計差"}),
    ("日付マスタ", "raw_date_master", "HARATO.RAW.RAW_DATE_MASTER", T_DATE_MASTER, {}),
    ("施設マスタ", "raw_facility_master", "HARATO.RAW.RAW_FACILITY_MASTER", T_FACILITY_MASTER, {}),
    ("施設名マッピング", "raw_facility_name_mappings", "HARATO.RAW.RAW_FACILITY_NAME_MAPPINGS", T_NAME_MAPPINGS, {}),
]
...
# ===== データ取得 =====
def bq_dataframe(client, table: str, cols: list[str]) -> pd.DataFrame:
    sql = f"SELECT {', '.join(cols)} FROM `{BQ_PROJECT}.{BQ_DATASET}.{table}`"
...
def main() -> int:
    ...
    results = []
    try:
        for label, bq_table, sf_fqtn, types, exclude in TABLES:
            if args.table and args.table != label:
                continue
            bq_df = bq_dataframe(bq, bq_table, list(types))
            sf_df = sf_dataframe(sf, sf_fqtn)
            results.append(compare(label, bq_df, sf_df, types, exclude, args.show_diff))
    finally:
        sf.close()
    print("総合:", "★ 全テーブル一致" if all(results) else "差分あり")
